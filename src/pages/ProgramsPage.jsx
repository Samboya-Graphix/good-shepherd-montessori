...
export default function ProgramsPage({ onOpenTourModal }) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-100 text-[#181b66] text-xs font-bold uppercase tracking-wider">
            <GraduationCap className="w-4 h-4 text-[#181b66]" />
            Ages 4 Months to 4 Years · Bechem
          </span>
          <h1 className="mt-4 text-3xl sm:text-4xl lg:text-5xl font-serif font-bold text-[#181b66] tracking-tight">
            Creche & Day Care Programs
          </h1>
          <p className="mt-4 text-base sm:text-lg text-slate-600 font-light leading-relaxed">
            Good Shepherd Montessori School currently focuses exclusively on foundational early childhood care, running specialized Creche and Day Care departments for children aged 4 Months to 4 Years.
          </p>
        </div>
        ...
        <div className="mt-10 bg-white rounded-3xl border border-slate-200 shadow-card overflow-hidden">
          {/* Header Banner */}
          <div className="bg-gradient-to-r from-[#181b66] via-[#1c2278] to-[#121447] text-white p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-4 border-blue-400">
            <div>
              <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-blue-200 mb-2">
                <span className="w-2 h-2 rounded-full bg-blue-300"></span>
                {activeProgram.badge} · Bechem Campus
              </div>
          ...
          <div className="p-6 sm:p-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            {/* Left Column: Curriculum Focus */}
            <div className="lg:col-span-7 space-y-6">
              <div>
                <h3 className="text-lg font-serif font-bold text-[#181b66] flex items-center gap-2">
                  <BookOpen className="w-5 h-5 text-[#181b66]" />
                  Curriculum & Core Areas
                </h3>
                <p className="text-xs sm:text-sm text-slate-500 mt-1">
                  Integrating concrete Montessori learning apparatus with the Ghana Education Service syllabus.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {activeProgram.curriculumAreas.map((area, idx) => (
                  <div 
                    key={idx} 
                    className="p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-blue-300 transition-colors"
                  >
              ...
              <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 text-sm text-slate-700">
                  ...
                  <span><strong>Drop-off & Pick-up:</strong> Parent arrival at campus</span>
                </div>
              </div>
        ...
        <div className="mt-12 rounded-2xl bg-white border border-blue-200 p-6 shadow-sm flex flex-col md:flex-row items-center gap-5">
          <div className="w-12 h-12 rounded-2xl bg-blue-50 text-[#181b66] border border-blue-200 flex items-center justify-center shrink-0">
            <Church className="w-6 h-6 text-[#181b66]" />
          </div>
          <div className="flex-1 text-center md:text-left">
            <div className="text-xs font-bold uppercase tracking-wider text-blue-600">
              {expansionNotice.title} · {expansionNotice.scope}
            </div>
            <p className="text-xs sm:text-sm text-slate-600 mt-1 font-light leading-relaxed">
              {expansionNotice.description}
            </p>
          </div>
          <Link
            to="/admissions"
            className="px-5 py-2.5 rounded-xl bg-[#181b66] hover:bg-blue-900 text-white font-bold text-xs shrink-0 transition-colors shadow-sm"
          >
        ...
        <div className="mt-16 pt-12 border-t border-slate-200">
          <div className="text-center max-w-2xl mx-auto mb-10">
            ...
            <h3 className="mt-3 text-2xl sm:text-3xl font-serif font-bold text-[#181b66]">
              Why Choose Good Shepherd
            </h3>
            <p className="mt-2 text-xs sm:text-sm text-slate-600 font-light">
              Built upon 4 core values ensuring every child aged 4 Months to 4 Years thrives.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {whyChooseUs.map((pillar) => {
              ...
              return (
                <div 
                  key={pillar.id}
                  className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs hover:border-blue-400 transition-all text-left"
                >
                  <div className="w-9 h-9 rounded-xl bg-blue-50 text-[#181b66] flex items-center justify-center mb-3">
